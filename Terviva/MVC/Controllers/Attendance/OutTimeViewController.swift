...
class OutTimeViewController: UIViewController {
    ...
    @IBAction func submitButtonTapped(_ sender: UIButton) {
        if self.base64Image != "" && self.commentTextField.text != "" {
             self.sendOutTimeDeailsToServerMethod(pickedImage: self.base64Image, comment: self.commentTextField.text ?? "")
        } else {
    ...
    func sendOutTimeDeailsToServerMethod(pickedImage: String, comment: String) {
        SGCommonMethods.SharedInstance.showLoader()
        let outTimeUrl = AppUrl.BaseUrl + MethodUrl.attendanceEntryUrl
        ...
        let params = ["associate_id":id,"tl_id":isd_code,"outlet_id":"","attendance_type":"out","attendance_image": pickedImage,"latitude":latitudeStr,"longitude":longitudeStr,"distance":"","attendance_date":currentDate,"reason":comment,"remarks":comment,"leave_type":""]
        NetworkingManager.instance.sendDetailsToServerMethod(parametersDict: params, imageDataDic: [:], baseUrl: outTimeUrl) { responseDic in
            print(responseDic)
            SGCommonMethods.SharedInstance.hideLoader()
            let status = responseDic["status"] as? Bool ?? false
            if status == true {
                DispatchQueue.main.async {
                    self.showAlertWithTitle(withTitle: projectName, andMessage: OutTimeUpdate) { success in
...
extension OutTimeViewController: UIImagePickerControllerDelegate,UINavigationControllerDelegate {
...
extension OutTimeViewController: UIDocumentPickerDelegate {
